...
router.post("/", (req, res) => {
  ...
  import_application_svc_mongo.default.create(newApplicationData).then(
...
router.put("/:id", (req, res) => {
  ...
  import_application_svc_mongo.default.update(id, updatedApplicationData).then((updatedApplicationData2) => res.json(updatedApplicationData2)).catch((err) => res.status(404).end());
});
router.delete("/:id", (req, res) => {
  const { id } = req.params;
  import_application_svc_mongo.default.remove(id).then(() => res.status(204).end()).catch((err) => res.status(404).send(err));
});
